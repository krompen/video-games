...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        try:
            ...
            pass
        try:
            await db.execute("ALTER TABLE users ADD COLUMN status TEXT DEFAULT 'user'")
        except aiosqlite.OperationalError:
            pass
        try:
            await db.execute("ALTER TABLE users ADD COLUMN internal_id TEXT")
        except aiosqlite.OperationalError:
            pass
...
async def get_user_info(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute(
            "SELECT diamonds, is_banned, is_manager, referrer_id, status FROM users WHERE user_id = ?", (user_id,)
        )).fetchone()
...
async def give_referral_bonus(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        referrer_id = row[0]
        # Даём 10 алмазиков и приглашённому, и пригласившему
        await db.execute(
        ...
        try:
            ...
            await bot.send_message(user_id, f"🎉 <b>Реферальный бонус!</b>\n\n+{REFERRAL_BONUS} 💎 за приглашение")
        except:
            pass
...
def founder_menu():
    return admin_menu()  # Полный доступ как у основателя
...
async def support_save(message: Message, state: FSMContext):
    user_id = message.from_user.id
    async with aiosqlite.connect(DB_NAME) as db:
        open_ticket = await (await db.execute(
            "SELECT id FROM support_tickets WHERE user_id = ? AND status = 'open' LIMIT 1", (user_id,)
        )).fetchone()
        if open_ticket:
...
async def view_user_from_list(callback: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute(
            "SELECT user_id, username, diamonds, is_banned, is_manager, referrer_id, created_at, internal_id, status FROM users WHERE user_id = ?", (uid,)
        )).fetchone()
        if not row:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💎 Выдать алмазики", callback_data=f"give_diamonds_{uid}"),
         InlineKeyboardButton(text="💎 Обнулить", callback_data=f"reset_diamonds_{uid}")],
...
async def show_tickets_page(message, page: int = 0, is_manager: bool = False):
    ...
    if not tickets:
        ...
        text = f"🎟 <b>Активные тикеты</b> (стр. {page+1}/{total_pages})\n\nНажми на тикет 📖 чтобы прочитать историю сообщений, ответить или закрыть."
        kb_list = []
        for t in tickets:
...
async def view_ticket(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        messages = await (await db.execute(
            "SELECT user_id, message, is_admin FROM support_messages WHERE ticket_id = ? ORDER BY id ASC LIMIT 20",
            (ticket_id,)
        )).fetchall()
    ...
    if not messages:
        ...
        if len(messages) == 20:
            text += "📌 Показаны последние 20 сообщений. Для полной истории используй /start и мои тикеты."
...
async def save_pack_content(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await message.answer("✅ Контент успешно добавлен в пак! Нажми кнопку ниже, чтобы обновить меню управления.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📦 Обновить меню управления паком", callback_data=f"pack_manage_{pack_id}")],
        [InlineKeyboardButton(text="◀️ Назад к пакам", callback_data="admin_packs")]
    ]))
    await state.clear()
