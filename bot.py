...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, diamonds INTEGER DEFAULT 0, last_bonus TEXT, is_banned INTEGER DEFAULT 0, is_manager INTEGER DEFAULT 0)""")
...
async def start(message: Message):
    ...
    await message.answer("🎮 <b>Video Games Bot</b>\n\nСмотри фото и видео за алмазики!", reply_markup=main_menu(message.from_user.id))
...
async def profile(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"👤 <b>Профиль</b>\n\n💎 Алмазики: <b>{diamonds}</b>", reply_markup=main_menu(callback.from_user.id))
...
async def watch_photo(callback: CallbackQuery):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await bot.send_photo(callback.from_user.id, content[0], caption=f"📸 Вот фото!\n💎 Баланс: {diamonds-1}")
...
async def watch_video(callback: CallbackQuery):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await bot.send_video(callback.from_user.id, content[0], caption=f"🎥 Вот видео!\n💎 Баланс: {diamonds-2}")
