...
async def give_referral_bonus(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.execute("UPDATE users SET diamonds = diamonds + ? WHERE user_id = ?", (REFERRAL_BONUS, row[0]))
        await db.commit()
        try:
...
async def bulk_save_content(message: Message, state: FSMContext):
    ...
    try:
        await bot.edit_message_text(
            ...
            text=f"📤 <b>МАССОВАЯ ЗАГРУЗКА</b>\n\n"
                 f"Сохранено: <b>{new_count}</b> файлов\n"
                 f"Тип последнего: {content_type}\n\n"
                 f"Продолжай кидать контент или напиши /start для выхода.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="✅ Завершить загрузку", callback_data="bulk_upload_finish")]
            ])
        )
    ...
    await asyncio.sleep(0.75)  # Защита от flood
...
async def main():
    ...
    logger.info("🚀 Bot v10 Bulk Upload запущен! Массовый режим готов.")
