...
async def watch_photo(callback: CallbackQuery):
    ...
    info = await get_user_info(user_id)
    if info["diamonds"] < 1:
        await callback.message.edit_text("❌ Нужно минимум 1 💎", reply_markup=await main_menu(user_id))
        return
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await bot.send_photo(user_id, content[0], caption=f"📸 Вот фото!\n💎 Остаток: {info['diamonds']-1}")
...
async def watch_video(callback: CallbackQuery):
    ...
    info = await get_user_info(user_id)
    if info["diamonds"] < 2:
        await callback.message.edit_text("❌ Нужно минимум 2 💎", reply_markup=await main_menu(user_id))
        return
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await bot.send_video(user_id, content[0], caption=f"🎥 Вот видео!\n💎 Остаток: {info['diamonds']-2}")
    await bot.send_message(user_id, "Что дальше?", reply_markup=await main_menu(user_id))
...
async def shop(callback: CallbackQuery):
    await callback.message.edit_text("🛒 Напиши в поддержку — предложим выгодные пакеты алмазиков.", reply_markup=await main_menu(callback.from_user.id))
...
async def support_save(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await message.answer(f"✅ Тикет #{ticket_id} создан! Мы ответим в ближайшее время.")
    await state.clear()
    await message.answer("🎮 Меню:", reply_markup=await main_menu(user_id))

# ==================== МОИ ТИКЕТЫ (НОВАЯ ФУНКЦИЯ) ====================
@dp.callback_query(F.data == "my_tickets")
async def my_tickets(callback: CallbackQuery):
    user_id = callback.from_user.id
    async with aiosqlite.connect(DB_NAME) as db:
        tickets = await (await db.execute(
            "SELECT id, status FROM support_tickets WHERE user_id = ? ORDER BY id DESC LIMIT 10", (user_id,)
        )).fetchall()
    
    if not tickets:
        text = "🎟 <b>У тебя пока нет тикетов.</b>\n\nНапиши в поддержку — создадим первый!"
        await callback.message.edit_text(text, reply_markup=await main_menu(user_id))
        return
...
async def close_my_ticket(callback: CallbackQuery):
    ...
    await my_tickets(callback)  # обновляем список
...
async def admin_add_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("➕ Введи: @канал награда (пример: @channel 10)")
...
async def admin_mass_give_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("💎 Введи сколько алмазиков выдать ВСЕМ активным пользователям:")
...
async def admin_mass_give(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            await db.commit()
        await message.answer(f"✅ +{amount} 💎 успешно выдано всем!")
    except:
...
async def admin_give_manager_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Введи ID пользователя для выдачи статуса менеджера:")
...
async def admin_remove_manager_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Введи ID пользователя для снятия статуса менеджера:")
...
async def show_tickets_page(message, page: int = 0):
    ...
    if not tickets:
        ...
        nav = []
        if page > 0: nav.append(InlineKeyboardButton("◀️", callback_data=f"tickets_page_{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
        if page < total_pages-1: nav.append(InlineKeyboardButton("▶️", callback_data=f"tickets_page_{page+1}"))
...
async def reply_ticket_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"✍️ Напиши ответ на тикет #{ticket_id}:")
...
async def manager_ban(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🚫 Введи ID для бана/разбана:")
...
async def give_diamonds_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        try: await bot.send_message(target, f"🎉 +{amount} 💎 от команды!")
        except: pass
    except:
        await message.answer("❌ Ошибка")
    await state.clear()
...
async def main():
    await init_db()
    logger.info("🚀 Bot v4 с системой тикетов запущен!")
