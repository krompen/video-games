...
async def watch_photo(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if diamonds < 1:
            return await callback.answer("❌ Недостаточно алмазиков! Нужно 1 💎", show_alert=True)
    ...
    await bot.send_photo(user_id, content[0], caption="📸 Вот твоё фото!")
...
async def watch_video(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if diamonds < 2:
            return await callback.answer("❌ Недостаточно алмазиков! Нужно 2 💎", show_alert=True)
    ...
    await bot.send_video(user_id, content[0], caption="🎥 Вот твоё видео!")
