...
OWNER_ID = int(os.getenv("OWNER_ID", "8032626504"))
DB_NAME = "video_games_bot.db"
REQUIRED_CHANNEL_ID = int(os.getenv("REQUIRED_CHANNEL_ID", "-1003969378970"))
REQUIRED_CHANNEL_LINK = os.getenv("REQUIRED_CHANNEL_LINK", "https://t.me/rezerv_video_pita")
REFERRAL_BONUS = 10
...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS users (
            ...
            referral_paid INTEGER DEFAULT 0, status TEXT DEFAULT 'user', internal_id TEXT UNIQUE
        )""")
        for col, default in [("referral_paid", "0"), ("status", "'user'"), ("internal_id", "NULL")]:
            try:
                await db.execute(f"ALTER TABLE users ADD COLUMN {col} {default.split()[0] if default != 'NULL' else 'TEXT'} DEFAULT {default}")
            except:
                pass
...
async def log_action(actor_id: int, action: str, target_id: int = None, details: str = ""):
    try:
        async with aiosqlite.connect(DB_NAME) as db:
            await db.execute(
                "INSERT INTO admin_logs (actor_id, action, target_id, details) VALUES (?, ?, ?, ?)",
...
class AdminBroadcast(StatesGroup):
    waiting_text = State()
class StaffBroadcast(StatesGroup):
    waiting_text = State()
...
class AdminAddPackContent(StatesGroup):
    pack_id = State()
    content_type = State()
class SupportWriteChannel(StatesGroup):
    waiting_text = State()
...
async def get_user_info(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute(
            "SELECT diamonds, is_banned, is_manager, referrer_id, status, created_at, internal_id FROM users WHERE user_id = ?", (user_id,)
        )).fetchone()
        if row:
            return {"diamonds": row[0], "is_banned": row[1], "is_manager": row[2], "referrer_id": row[3], "status": row[4], "created_at": row[5], "internal_id": row[6]}
        return {"diamonds": 0, "is_banned": 0, "is_manager": 0, "referrer_id": None, "status": "user", "created_at": None, "internal_id": None}
...
async def is_admin_or_manager(user_id: int) -> bool:
    ...
    info = await get_user_info(user_id)
    return info.get("is_manager", 0) == 1 or info.get("status") in ["founder", "support", "manager"]

async def get_user_status(user_id: int) -> str:
    if user_id == OWNER_ID: return "founder"
    info = await get_user_info(user_id)
    return info.get("status", "user")

async def get_or_create_user(user_id: int, username: str = None, referrer_id: int = None):
    async with aiosqlite.connect(DB_NAME) as db:
        existing = await (await db.execute("SELECT user_id, referrer_id FROM users WHERE user_id = ?", (user_id,))).fetchone()
        if existing:
            if referrer_id and existing[1] is None:
                await db.execute("UPDATE users SET referrer_id = ? WHERE user_id = ?", (referrer_id, user_id))
                await db.commit()
            return
        internal_id = await generate_internal_id()
        await db.execute("INSERT INTO users (user_id, username, referrer_id, internal_id) VALUES (?, ?, ?, ?)", (user_id, username or "", referrer_id, internal_id))
        await db.commit()

async def give_referral_bonus(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("BEGIN TRANSACTION")
        row = await (await db.execute("SELECT referrer_id, COALESCE(referral_paid, 0) FROM users WHERE user_id = ?", (user_id,))).fetchone()
        if not row or not row[0] or row[1] == 1:
            await db.execute("ROLLBACK")
            return
        referrer_id = row[0]
        ref_info = await (await db.execute("SELECT is_banned FROM users WHERE user_id = ?", (referrer_id,))).fetchone()
        if not ref_info or ref_info[0] == 1:
            await db.execute("ROLLBACK")
            return
        await db.execute("UPDATE users SET diamonds = diamonds + ?, referral_paid = 1 WHERE user_id = ?", (REFERRAL_BONUS, user_id))
        await db.execute("UPDATE users SET diamonds = diamonds + ? WHERE user_id = ?", (REFERRAL_BONUS, referrer_id))
        await db.commit()
        await log_action(user_id, "referral_bonus", target_id=referrer_id, details=f"+{REFERRAL_BONUS}")
        try:
            await bot.send_message(referrer_id, f"🎉 <b>Новый реферал!</b>\n\n+{REFERRAL_BONUS} 💎 (тебе и другу)")
            await bot.send_message(user_id, f"🎉 <b>Реферальный бонус начислен!</b>\n\n+{REFERRAL_BONUS} 💎 за приглашение")
        except Exception as e:
...
async def generate_internal_id() -> str:
...
async def main_menu(user_id: int):
    ...
    if status == "founder":
        keyboard.append([InlineKeyboardButton(text="🔧 ПАНЕЛЬ ОСНОВАТЕЛЯ", callback_data="founder_menu")])
    elif status == "support":
        keyboard.append([InlineKeyboardButton(text="🛠 ПАНЕЛЬ ТЕХПОДДЕРЖКИ", callback_data="support_menu")])
    elif status == "manager":
        keyboard.append([InlineKeyboardButton(text="🛡️ ПАНЕЛЬ МЕНЕДЖЕРА", callback_data="manager_menu")])
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📢 Рассылка персоналу", callback_data="staff_broadcast_start"),
...
def support_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎟 Тикеты", callback_data="support_tickets"),
         InlineKeyboardButton(text="✉️ Написать в канал", callback_data="support_write_channel")],
        [InlineKeyboardButton(text="📊 Статистика", callback_data="manager_stats"),
         InlineKeyboardButton(text="◀️ Назад", callback_data="main_menu")]
    ])
...
async def profile(callback: CallbackQuery):
    ...
    info = await get_user_info(user_id)
    status_map = {"founder": "👑 Основатель", "support": "🛠 Техподдержка", "manager": "🛡️ Менеджер", "user": "👤 Пользователь"}
    status = status_map.get(info.get("status", "user"), "👤 Пользователь")
    if user_id == OWNER_ID: status = "👑 Основатель"
    ref_count = 0
    async with aiosqlite.connect(DB_NAME) as db:
...
# ==================== ИСПРАВЛЕННЫЕ watch_photo / watch_video ====================
...
async def watch_photo(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if not content:
            ...
            return
        try:
            await bot.send_photo(user_id, content[0], caption="📸 Фото!")
            await db.execute("UPDATE users SET diamonds = diamonds - 1 WHERE user_id = ?", (user_id,))
            await db.commit()
            await bot.send_message(user_id, f"💎 Остаток: {info['diamonds']-1}", reply_markup=await main_menu(user_id))
        except Exception as e:
...
async def watch_video(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if not content:
            ...
            return
        try:
            await bot.send_video(user_id, content[0], caption="🎥 Видео!")
            await db.execute("UPDATE users SET diamonds = diamonds - 2 WHERE user_id = ?", (user_id,))
            await db.commit()
            await bot.send_message(user_id, f"💎 Остаток: {info['diamonds']-2}", reply_markup=await main_menu(user_id))
        except Exception as e:
...
async def shop(callback: CallbackQuery):
    await callback.message.answer(
        ...
        "Для покупки напиши в поддержку — мы поможем!\n"
        "💎 Цены и условия обсуждаются индивидуально.",
...
async def support_save(message: Message, state: FSMContext):
    user_id = message.from_user.id
    async with aiosqlite.connect(DB_NAME) as db:
        open_ticket = await (await db.execute("SELECT id FROM support_tickets WHERE user_id = ? AND status = 'open' LIMIT 1", (user_id,))).fetchone()
        if open_ticket:
            ...
            action = "добавлено в существующий"
        else:
            ...
            action = "создан новый"
        await db.commit()
    await message.answer(f"✅ Сообщение {action} в тикет #{ticket_id}!")
    await state.clear()
    await message.answer("🎮 Меню:", reply_markup=await main_menu(user_id))
...
async def bulk_upload_start(callback: CallbackQuery, state: FSMContext):
    ...
    msg = await callback.message.edit_text("📤 <b>МАССОВАЯ ЗАГРУЗКА АКТИВИРОВАНА</b>\n\nКидай фото и видео подряд.\nЧтобы завершить — напиши /start или нажми кнопку.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ Завершить", callback_data="bulk_upload_finish")]]))
...
async def bulk_save_content(message: Message, state: FSMContext):
    ...
    try:
        await bot.edit_message_text(chat_id=message.chat.id, message_id=data["counter_msg_id"], text=f"📤 Сохранено: <b>{new_count}</b> файлов", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ Завершить", callback_data="bulk_upload_finish")]]))
    except:
        pass
...
async def bulk_upload_finish(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.message.edit_text(f"✅ Загрузка завершена! Сохранено: <b>{count}</b> файлов.", reply_markup=admin_menu())
...
async def exit_bulk_upload(message: Message, state: FSMContext):
    ...
    count = data.get("count", 0)
    await state.clear()
    await message.answer(f"✅ Массовый режим завершён. Сохранено: <b>{count}</b> файлов.", reply_markup=await main_menu(message.from_user.id))
...
async def save_content(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("INSERT INTO content (file_id, content_type, added_by) VALUES (?, ?, ?)", (file_id, content_type, message.from_user.id))
        await db.commit()
    await message.answer(f"✅ {content_type.capitalize()} сохранено!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="admin_menu")]]))
    await state.clear()
...
async def admin_tasks(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="➕ Добавить", callback_data="admin_add_task"), InlineKeyboardButton(text="🗑 Удалить", callback_data="admin_delete_task")], [InlineKeyboardButton(text="◀️ Назад", callback_data="admin_menu")]])
...
async def admin_delete_task(callback: CallbackQuery):
    ...
    if not tasks:
        ...
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"🗑 #{t[0]} {t[1]}", callback_data=f"delete_task_{t[0]}")] for t in tasks] + [[InlineKeyboardButton(text="◀️ Назад", callback_data="admin_tasks")]])
...
async def admin_all_users(callback: CallbackQuery, page: int = 0):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        users = await (await db.execute("SELECT user_id, username, diamonds, is_banned, is_manager FROM users ORDER BY user_id DESC LIMIT ? OFFSET ?", (USERS_PER_PAGE, offset))).fetchall()
        total = (await (await db.execute("SELECT COUNT(*) FROM users")).fetchone())[0]
...
async def view_user_from_list(callback: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute("SELECT user_id, username, diamonds, is_banned, is_manager, referrer_id, created_at, internal_id, status FROM users WHERE user_id = ?", (uid,))).fetchone()
        if not row:
            await callback.answer("Пользователь не найден", show_alert=True)
            return
        ref_count = (await (await db.execute("SELECT COUNT(*) FROM users WHERE referrer_id = ?", (uid,))).fetchone())[0]
        status_map = {"founder": "👑 Основатель", "support": "🛠 Техподдержка", "manager": "🛡️ Менеджер", "user": "👤 Пользователь"}
        status_text = status_map.get(row[8], "👤 Пользователь")
        text = f"👤 <b>Пользователь {uid}</b>\n\n🆔 Внутренний ID: <code>{row[7]}</code>\n💎 Алмазики: <b>{row[2]}</b>\n🚫 Забанен: {'Да' if row[3] else 'Нет'}\n📌 Статус: {status_text}\n👥 Рефералов: <b>{ref_count}</b>"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💎 Выдать алмазики", callback_data=f"give_diamonds_{uid}"), InlineKeyboardButton(text="💎 Обнулить", callback_data=f"reset_diamonds_{uid}")],
        [InlineKeyboardButton(text="🔄 Сменить статус", callback_data=f"change_status_{uid}")],
        [InlineKeyboardButton(text="🚫 Забанить/Разбанить", callback_data=f"ban_user_{uid}")],
...
async def admin_view_user(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            row = await (await db.execute("SELECT user_id, username, diamonds, is_banned, is_manager, referrer_id, created_at FROM users WHERE user_id = ?", (uid,))).fetchone()
            if not row:
...
async def admin_mass_give(message: Message, state: FSMContext):
    ...
    try:
        ...
        await log_action(message.from_user.id, "mass_give", details=f"+{amount}")
    except:
        await message.answer("❌ Ошибка")
    await state.clear()
    await message.answer("🔧 Админ-панель:", reply_markup=admin_menu())
...
async def admin_manager_menu(callback: CallbackQuery):
    if callback.from_user.id != OWNER_ID: return
    await callback.message.edit_text("👑 <b>Управление менеджерами</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ Выдать статус", callback_data="admin_give_manager"), InlineKeyboardButton(text="❌ Снять статус", callback_data="admin_remove_manager")], [InlineKeyboardButton(text="◀️ Назад", callback_data="admin_menu")]]))
...
async def show_tickets_page(message, page: int = 0, is_manager: bool = False):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        tickets = await (await db.execute("SELECT id, user_id FROM support_tickets WHERE status='open' ORDER BY id DESC LIMIT ? OFFSET ?", (TICKETS_PER_PAGE, offset))).fetchall()
...
async def view_ticket(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        messages = await (await db.execute("SELECT user_id, message, is_admin FROM support_messages WHERE ticket_id = ? ORDER BY id ASC LIMIT 20", (ticket_id,))).fetchall()
    ...
    if not messages:
        ...
        for msg in messages:
            ...
            msg_text = msg[1][:150] + "..." if len(msg[1]) > 150 else msg[1]
            text += f"{prefix} {msg_text}\n\n"
...
async def reply_ticket_start(callback: CallbackQuery, state: FSMContext):
    ...
    actor_status = await get_user_status(callback.from_user.id)
    role_map = {"founder": "👑 Основатель", "support": "🛠 Техподдержка", "manager": "🛡️ Менеджер"}
    role = role_map.get(actor_status, "🛡️ Персонал")
    async with aiosqlite.connect(DB_NAME) as db:
        user_id = (await (await db.execute("SELECT user_id FROM support_tickets WHERE id=?", (ticket_id,))).fetchone())[0]
    try:
        await bot.send_message(user_id, f"🛠 <b>{role}</b> взял тикет #{ticket_id} в работу.")
    except:
        pass
...
async def reply_ticket_send(message: Message, state: FSMContext):
    ...
    actor_status = await get_user_status(message.from_user.id)
    role_map = {"founder": "👑 Основатель", "support": "🛠 Техподдержка", "manager": "🛡️ Менеджер"}
    role = role_map.get(actor_status, "🛡️ Персонал")
    async with aiosqlite.connect(DB_NAME) as db:
    ...
    await log_action(message.from_user.id, "ticket_reply", target_id=user_id, details=f"Тикет #{ticket_id}")
    await state.clear()
    await message.answer("🔧 Админ-панель:", reply_markup=admin_menu())
...
async def give_diamonds_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await log_action(user_id, "give_diamonds", target_id=target, details=f"+{amount}")
    except:
        await message.answer("❌ Ошибка")
    await state.clear()
    if user_id == OWNER_ID:
    ...
    try:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            if row:
                ...
                try:
                    if new == 1:
                        await bot.ban_chat_member(REQUIRED_CHANNEL_ID, uid)
                    else:
                        await bot.unban_chat_member(REQUIRED_CHANNEL_ID, uid)
                except:
                    pass
    except:
        await message.answer("❌ Ошибка")
...
async def create_promo_max_uses(message: Message, state: FSMContext):
    ...
    try:
        async with aiosqlite.connect(DB_NAME) as db:
            await db.execute("INSERT INTO promo_codes (code, diamonds, max_uses, created_by) VALUES (?, ?, ?, ?)", (data["code"], data["diamonds"], int(message.text), message.from_user.id))
            await db.commit()
...
async def show_pack_manage(chat_id: int, pack_id: int):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✏️ Изменить название", callback_data=f"edit_pack_name_{pack_id}"), InlineKeyboardButton(text="✏️ Изменить цену", callback_data=f"edit_pack_price_{pack_id}")],
        [InlineKeyboardButton(text="✏️ Изменить описание", callback_data=f"edit_pack_desc_{pack_id}"), InlineKeyboardButton(text="🔄 Переключить активность", callback_data=f"toggle_pack_{pack_id}")],
        [InlineKeyboardButton(text="➕ Добавить фото", callback_data=f"add_pack_photo_{pack_id}"), InlineKeyboardButton(text="➕ Добавить видео", callback_data=f"add_pack_video_{pack_id}")],
...
async def buy_pack(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await callback.answer(f"✅ Пак «{pack[1]}» куплен за {price} 💎!", show_alert=True)
...
async def my_packs(callback: CallbackQuery):
    user_id = callback.from_user.id
    async with aiosqlite.connect(DB_NAME) as db:
        packs = await (await db.execute("SELECT p.id, p.name, p.description, p.price, up.purchased_at FROM user_packs up JOIN packs p ON up.pack_id = p.id WHERE up.user_id = ? ORDER BY up.purchased_at DESC", (user_id,))).fetchall()
    if not packs:
...
async def view_pack_content(callback: CallbackQuery):
    ...
    if not contents:
        ...
        return
    await callback.answer(f"📤 Отправляю {len(contents)} файлов...", show_alert=True)
    media_items = []
    ...
    for i in range(0, len(media_items), 10):
        ...
        try:
            ...
            logger.warning(f"Media group error: {e}")
...
async def save_pack_content(message: Message, state: FSMContext):
    ...
    if ctype == "photo" and message.photo:
        ...
        return
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("INSERT INTO pack_content (pack_id, file_id, content_type, added_by) VALUES (?, ?, ?, ?)", (pack_id, file_id, ctype, message.from_user.id))
        await db.commit()
    await message.answer("✅ Контент добавлен в пак!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📦 Обновить меню", callback_data=f"pack_manage_{pack_id}")], [InlineKeyboardButton(text="◀️ Назад к пакам", callback_data="admin_packs")]]))
    await state.clear()
...
async def admin_create_pack_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("➕ Введите название нового пака:", reply_markup=cancel_keyboard())
...
async def create_pack_name(message: Message, state: FSMContext):
    ...
    await message.answer("📝 Введите описание (или /skip):", reply_markup=cancel_keyboard())
...
async def create_pack_desc(message: Message, state: FSMContext):
    ...
    await message.answer("💰 Введите цену в алмазиках (минимум 20):", reply_markup=cancel_keyboard())
...
async def create_pack_price(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("INSERT INTO packs (name, description, price) VALUES (?, ?, ?)", (data["name"], data["desc"], price))
        await db.commit()
    await state.clear()
    await message.answer(f"✅ Пак «{data['name']}» создан за {price} 💎!", reply_markup=admin_menu())
...
async def change_status_start(callback: CallbackQuery):
    ...
    uid = int(callback.data.split("_")[2])
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👑 Основатель", callback_data=f"set_status_{uid}_founder")],
        [InlineKeyboardButton(text="🛠 Техподдержка", callback_data=f"set_status_{uid}_support")],
        [InlineKeyboardButton(text="🛡️ Менеджер", callback_data=f"set_status_{uid}_manager")],
        [InlineKeyboardButton(text="👤 Пользователь", callback_data=f"set_status_{uid}_user")],
        [InlineKeyboardButton(text="◀️ Отмена", callback_data="admin_all_users")]
    ])
    await callback.message.edit_text(f"Выбери новый статус для {uid}:", reply_markup=kb)
...
async def set_status(callback: CallbackQuery):
    ...
    await log_action(callback.from_user.id, "change_status", target_id=uid, details=f"Новый статус: {new_status}")
    await admin_all_users(callback)

# ==================== ЛОГИ ====================
@dp.callback_query(F.data == "admin_logs")
async def admin_logs(callback: CallbackQuery, page: int = 0):
    if callback.from_user.id != OWNER_ID: return
    LOGS_PER_PAGE = 8
    offset = page * LOGS_PER_PAGE
    async with aiosqlite.connect(DB_NAME) as db:
        logs = await (await db.execute("SELECT timestamp, actor_id, action, target_id, details FROM admin_logs ORDER BY id DESC LIMIT ? OFFSET ?", (LOGS_PER_PAGE, offset))).fetchall()
        total = (await (await db.execute("SELECT COUNT(*) FROM admin_logs")).fetchone())[0]
    total_pages = max(1, (total + LOGS_PER_PAGE - 1) // LOGS_PER_PAGE)
    text = f"📜 <b>Логи</b> (стр. {page+1}/{total_pages})\n\n"
    if not logs:
        text += "Пока нет записей."
    else:
        for log in logs:
            ts = log[0][5:16] if log[0] else "—"
            action_short = log[2][:18]
            line = f"• {ts} | {log[1]} → {action_short}"
            if log[3]: line += f" →{log[3]}"
            if log[4]: line += f" | {log[4][:25]}"
            text += line + "\n"
    kb_list = []
    nav = []
    if page > 0: nav.append(InlineKeyboardButton(text="◀️", callback_data=f"logs_page_{page-1}"))
    nav.append(InlineKeyboardButton(text=f"{page+1}/{total_pages}", callback_data="noop"))
...
async def logs_page(callback: CallbackQuery):
    if callback.from_user.id != OWNER_ID: return
    page = int(callback.data.split("_")[2])
    await admin_logs(callback, page)

# ==================== РАССЫЛКА ПЕРСОНАЛУ ====================
@dp.callback_query(F.data == "staff_broadcast_start")
async def staff_broadcast_start(callback: CallbackQuery, state: FSMContext):
    if not await is_admin_or_manager(callback.from_user.id):
        await callback.answer("❌ Нет прав!", show_alert=True)
        return
    await state.set_state(StaffBroadcast.waiting_text)
    await callback.message.edit_text("📢 Введи текст рассылки <b>только для персонала</b>:", reply_markup=cancel_keyboard())

@dp.message(StaffBroadcast.waiting_text)
async def staff_broadcast_send(message: Message, state: FSMContext):
    if not await is_admin_or_manager(message.from_user.id): return
    async with aiosqlite.connect(DB_NAME) as db:
        staff = await (await db.execute("SELECT user_id FROM users WHERE is_banned=0 AND (is_manager=1 OR status IN ('founder','support','manager'))")).fetchall()
    count = 0
    for (uid,) in staff:
        try:
            await bot.send_message(uid, f"📢 <b>Сообщение для персонала:</b>\n\n{message.text}")
            count += 1
            await asyncio.sleep(0.05)
        except: pass
    await message.answer(f"✅ Разослано {count} сотрудникам")
    await log_action(message.from_user.id, "staff_broadcast", details=f"Отправлено {count}")
    await state.clear()
    await message.answer("👑 Панель:", reply_markup=admin_menu())

# ==================== ПОДДЕРЖКА: НАПИСАТЬ В КАНАЛ ====================
@dp.callback_query(F.data == "support_write_channel")
async def support_write_channel_start(callback: CallbackQuery, state: FSMContext):
    if not await is_admin_or_manager(callback.from_user.id): return
    await state.set_state(SupportWriteChannel.waiting_text)
    await callback.message.edit_text("✉️ Введите сообщение для канала:", reply_markup=cancel_keyboard())

@dp.message(SupportWriteChannel.waiting_text)
async def support_write_channel_send(message: Message, state: FSMContext):
    if not await is_admin_or_manager(message.from_user.id): return
    try:
        await bot.send_message(REQUIRED_CHANNEL_ID, f"📢 <b>Сообщение от поддержки:</b>\n\n{message.text}")
        await message.answer("✅ Сообщение отправлено в канал!")
        await log_action(message.from_user.id, "write_to_channel", details=message.text[:50])
    except Exception as e:
        await message.answer(f"❌ Ошибка отправки: {e}")
    await state.clear()
    await message.answer("🛠 Панель поддержки:", reply_markup=support_menu())
...
async def main():
    ...
    logger.info("🚀 Bot v13 FULL (original style + all fixes) запущен!")
