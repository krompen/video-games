...
REFERRAL_BONUS = 10  # алмазики за приглашение
...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        # Add index for fast random selection
...
async def get_user_info(user_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        return {
            ...
            "is_banned": row[1] if row else 0,
            "is_manager": row[2] if row else 0,
        ...
        if referrer_id and referrer_id != user_id:
            ...
            try:
                await bot.send_message(
                    referrer_id,
                    f"🎉 <b>Новый реферал!</b>\n\n+{REFERRAL_BONUS} 💎 за приглашение пользователя!"
                )
            except:
                pass
...
async def save_content(message: Message, state: FSMContext):
    ...
    # Красивое сообщение с кнопкой назад
    await message.answer(
        f"✅ <b>{content_type.capitalize()} успешно сохранено!</b>",
...
async def admin_view_user_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Введи ID пользователя для просмотра статистики:", reply_markup=cancel_keyboard())
...
async def admin_view_user(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            # Count referrals
            referrals = await (await db.execute("SELECT COUNT(*) FROM users WHERE referrer_id = ?", (uid,))).fetchone()
            ref_count = referrals[0] if referrals else 0
            
            text = (
                f"👤 <b>Статистика пользователя</b>\n\n"
                f"🆔 ID: {row[0]}\n"
                f"👤 Username: @{row[1] or 'нет'}\n"
                f"💎 Алмазики: <b>{row[2]}</b>\n"
                f"🚫 Забанен: {'Да' if row[3] else 'Нет'}\n"
                f"🛡️ Менеджер: {'Да' if row[4] else 'Нет'}\n"
                f"👥 Рефералов: <b>{ref_count}</b>\n"
                f"📅 Регистрация: {row[6][:10] if row[6] else 'неизвестно'}"
            )
            if row[5]:
                text += f"\n🔗 Приглашён пользователем: {row[5]}"
...
async def main():
    ...
    logger.info("🚀 Bot v7 Full Optimized запущен! Рефка + Статистика + Оптимизация готовы.")
